        ...
        if ('car' in n['class']) or ('truck' in n['class']) or ('bus' in n['class']):
            ...
            if r:
                cv2.putText(fr,
                            ...
                            3,#thic
                            cv2.LINE_AA)
                file='/home/cctv/ppuimg/%s_%s.png'%(time.strftime('%d%m_%H:%M:%S'),r)
                cv2.imwrite(file,
                            fr)
                insert(r,file)
    busy=0
...
#---------------------
def con(q):
    cn = sqlite3.connect('/home/cctv/db/ppu.db')
    cr = cn.cursor()
    cr.execute(q)
    r = cr.fetchall()
    cn.commit()
    cn.close()
    return r
def insert(ppu,photo):
    tm=time.strftime('%H:%M:%S')
    q = 'INSERT or replace INTO data(ppu,time, photo) VALUES("%s","%s","%s");'%(ppu,tm,photo)
    r = con(q)
#---------------------
ip='192.168.100.69'
#ip='192.168.100.28'
