...
while 1:
    ...
    # revisar imagenes cada 500ms
    if time.time()>tmr and not(busy):
        tmr=time.time()+0.5
        for n in cnts:
            a=cv2.contourArea(n)
            # area minima para cosiderar movimiento detectado
            if (a>5000) and not(busy):
                th._start_new_thread(ocrResult,(f,))
    ##########################################
    #redimencionar para mostrar imagen
    #fx=f[100:500,400:1800]
    cv2.imshow('ima',grr)
